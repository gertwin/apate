...
APATE.Settings = (function settings() {
    ...
    fnConstructor.prototype = {
        ...
        async readsettings() {
            ...
            for (const key of Object.keys(CONST.SETTINGS)) {
        ...
        //     // read updated settings
        //     Object.keys(CONST.SETTINGS).forEach(async (key) => {
        //         const record = await this.storage.common.where({
        //             key,
        //         }).first();
        //         // update default value if found
        ...
        async set(key, value) {
            await this.storage.transaction('rw', [this.storage.common], async () => {
                ...
                if (oldValue) {
                    await this.storage.common.put({ key, value }).then(() => {
                    ...
                    await this.storage.common.add({ key, value }).then(() => {
                        this.settings[key] = value;
